 ...
 * ```
 */
export class FilesystemAttachmentStorage implements IAttachmentStorage {
  private storageDir: string;
  private createDirIfMissing: boolean;
  ...
  async initialize(): Promise<void> {
    if (this.createDirIfMissing) {
      ...
      throw new Error(
        `Attachment storage directory does not exist: ${this.storageDir}`,
      );
    }
  }
   ...
   */
  private calculateHash(base64Content: string): string {
  ...
  async storeAttachment(
    ...
    if (!fs.existsSync(filepath)) {
      ...
      console.log(
        `📎 Stored new attachment: ${attachment.filename} (hash: ${contentHash.substring(0, 8)}...)`,
      );
    } else {
      console.log(
        `📎 Attachment already exists (deduplicated): ${attachment.filename} (hash: ${contentHash.substring(0, 8)}...)`,
      );
    }
  ...
  async retrieveAttachment(
    ...
    try {
      ...
      throw new Error(
        `Failed to retrieve attachment ${metadata.filename} (hash: ${metadata.contentHash}): ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
 ...
 */
export function createDefaultAttachmentStorage(): FilesystemAttachmentStorage {
