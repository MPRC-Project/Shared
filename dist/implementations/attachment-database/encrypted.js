...
export class EncryptedAttachmentStorage {
    ...
    async storeAttachment(attachment) {
        // Encrypt the content
        const { encryptedAttachment, metadata: encryptedMeta } = await this.encryption.encryptAttachmentContent(attachment, 
    ...
    async retrieveAttachment(metadata) {
        ...
        const syntheticEncMeta = {
            id: metadata.id,
            contentHash: metadata.contentHash,
            size: metadata.size,
            contentEncrypted: true,
            encryptedMetadata: {}, // not needed for content decryption
        };
        // Decrypt the content
        const plaintextBase64 = await this.encryption.decryptAttachmentContent(ciphertextAttachment.content, syntheticEncMeta);
        return {
            id: metadata.id,
    ...
    async initialize() {
        ...
        console.log(`🔐 EncryptedAttachmentStorage initialized [backend: ${this.encryption.describe()}]`);
    }
    async close() {
        if (this.inner.close) {
 ...
 * ```
 */
export function createEncryptedAttachmentStorage(inner, encryption) {
