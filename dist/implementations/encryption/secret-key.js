 ...
 * ```
 */
export class SecretKeyEncryption {
    primaryKey;
    keyId;
    decryptionKeys;
    constructor(options) {
        ...
        if (this.primaryKey.length !== KEY_LENGTH_BYTES) {
            throw new Error(`SecretKeyEncryption: secret key must be ${KEY_LENGTH_BYTES} bytes, ` +
     ...
     */
    resolveDecryptionKey(keyId) {
        if (!keyId) {
            return this.primaryKey;
        }
        const key = this.decryptionKeys.get(keyId);
        if (!key) {
            throw new Error(`SecretKeyEncryption: no decryption key found for keyId "${keyId}". ` +
    ...
    static buildAAD(context) {
        ...
        if (context.entityType)
            parts.push(`entityType=${context.entityType}`);
        if (context.userId)
     ...
     */
    encryptBuffer(plaintext, key, aad) {
     ...
     */
    decryptBuffer(ciphertext, key, iv, authTag, aad) {
        ...
        try {
            ...
            throw new Error("SecretKeyEncryption: decryption failed — ciphertext may have been " +
     ...
     */
    buildEnvelope(iv, ciphertext, authTag, context) {
        return {
            algorithm: ALGORITHM,
     ...
     */
    unpackEnvelope(envelope) {
        if (envelope.algorithm !== ALGORITHM) {
            throw new Error(`SecretKeyEncryption: unsupported algorithm "${envelope.algorithm}". ` +
    ...
    async encryptString(plaintext, context) {
        ...
        const { iv, ciphertext, authTag } = this.encryptBuffer(Buffer.from(plaintext, "utf8"), this.primaryKey, aad);
        return this.buildEnvelope(iv, ciphertext, authTag, context);
    }
    async decryptString(encrypted, context) {
    ...
    async encryptBinary(base64Data, context) {
        const aad = SecretKeyEncryption.buildAAD(context);
        const { iv, ciphertext, authTag } = this.encryptBuffer(Buffer.from(base64Data, "base64"), this.primaryKey, aad);
        return this.buildEnvelope(iv, ciphertext, authTag, context);
    }
    async decryptBinary(encrypted, context) {
        const { key, iv, ciphertext, authTag } = this.unpackEnvelope(encrypted);
        const aad = SecretKeyEncryption.buildAAD(context);
        const plaintext = this.decryptBuffer(ciphertext, key, iv, authTag, aad);
    ...
    async encryptMessage(message) {
        ...
        if (message.attachments && message.attachments.length > 0) {
            encryptedAttachments = await Promise.all(message.attachments.map((att) => this.encryptAttachmentMetadata({
                id: att.id,
                filename: att.filename,
                ...(att.mimeType !== undefined ? { mimeType: att.mimeType } : {}),
            }, att.contentHash, att.size)));
        }
        return {
            id: message.id,
            from: message.from,
    ...
    // --------------------------------------------------------------------------
    async encryptAttachmentMetadata(attachment, contentHash, size) {
        const context = {
        ...
        const encryptedMetadata = await this.encryptString(JSON.stringify(metadataPayload), context);
        return {
            id: attachment.id,
            contentHash,
            size,
    ...
    async decryptAttachmentMetadata(encrypted) {
        const context = {
    ...
    async encryptAttachmentContent(attachment, contentHash) {
        const context = {
            entityId: attachment.id,
        ...
        const encryptedContentBase64 = Buffer.from(JSON.stringify(encryptedContentEnvelope), "utf8").toString("base64");
        const metadata = await this.encryptAttachmentMetadata({
            id: attachment.id,
            filename: attachment.filename,
            ...(attachment.mimeType !== undefined ? { mimeType: attachment.mimeType } : {}),
        }, contentHash, attachment.size ?? Buffer.byteLength(attachment.content, "base64"));
    ...
    async decryptAttachmentContent(encryptedContent, _metadata) {
 ...
 * ```
 */
export function createSecretKeyEncryption(secretKey, options) {
