 ...
 */
export function createSignaturePayload(command) {
 ...
 * ```
 */
export function signCommand(command, privateKey, publicKey) {
 ...
 * ```
 */
export function verifyAdminSignature(command, adminAuth) {
    try {
        // Check timestamp is not too old (prevent replay attacks)
        const now = Date.now();
        const age = now - adminAuth.timestamp;
        if (age > MAX_SIGNATURE_AGE_MS) {
            console.warn(`Admin signature too old: ${age}ms (max: ${MAX_SIGNATURE_AGE_MS}ms)`);
            return false;
        }
        ...
        const { adminAuth: _, ...commandWithoutAuth } = command;
        ...
        verify.end();
        const isValid = verify.verify(adminAuth.publicKey, adminAuth.signature, "base64");
        return isValid;
    }
    catch (error) {
 ...
 */
export function findAdminKey(publicKey, adminKeys) {
 ...
 */
export function validateAdminAuth(command, adminKeys) {
    // Check if this command requires auth
    if (!requiresAdminAuth(command)) {
        return true;
    }
    // Check if adminAuth is present
    const adminAuth = command.adminAuth;
    if (!adminAuth) {
        throw new Error(`Command ${command.command} requires admin authentication but none provided`);
    }
    // Find the admin key
    const adminKey = findAdminKey(adminAuth.publicKey, adminKeys);
    if (!adminKey) {
        throw new Error(`Admin public key not found in server configuration. Ensure the public key is registered in adminKeys.`);
    }
    // Verify the signature
    const isValid = verifyAdminSignature(command, adminAuth);
    if (!isValid) {
        throw new Error(`Invalid admin signature for key "${adminKey.name}". Signature verification failed.`);
    }
 ...
 * ```
 */
export function generateAdminKeyPair(keySize = 2048) {
